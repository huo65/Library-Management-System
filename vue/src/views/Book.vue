<template>
  <div class="home" style ="padding: 10px">
    ...
    <!-- 按钮-->
    <div style="margin: 10px 0;" >
      <el-button type="primary" @click = "add" v-if="user.role == 2">上架</el-button>
      <el-popconfirm title="确认下架?" @confirm="deleteBatch" v-if="user.role == 2">
        <template #reference>
...
<!--      <el-table-column prop="leftNumber" label="剩余数量" sortable/>-->
      ...
      <el-table-column fixed="right" label="操作" >
        <template v-slot="scope">
          <el-button  size="mini" @click ="handleEdit(scope.row)" v-if="user.role == 2">修改</el-button>
          <el-popconfirm title="确认下架?" @confirm="handleDelete(scope.row.id)" v-if="user.role == 2">
            <template #reference>
